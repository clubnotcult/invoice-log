...
export default async function handler(req, res) {
  ...
  try {
    switch (action) {
      ...
      // ── Void a Stripe invoice (draft/open only) ──────────────
      case 'void_invoice': {
        const { stripe_invoice_id } = payload;
        const existing = await stripeRequest('GET', `/invoices/${stripe_invoice_id}`);
        if (existing.status === 'draft') {
          await stripeRequest('DELETE', `/invoices/${stripe_invoice_id}`);
          return res.json({ voided: true, method: 'deleted' });
        }
        if (existing.status === 'open') {
          const voided = await stripeRequest('POST', `/invoices/${stripe_invoice_id}/void`);
          return res.json({ voided: true, method: 'voided', status: voided.status });
        }
        return res.json({ voided: false, reason: `Invoice status is ${existing.status} — cannot void` });
      }
